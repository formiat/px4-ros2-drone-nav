...
перед этой доработкой. Peer review выявил две реальные неоднозначности: unsafe suppression при
tie ground/solid и недостижимость ground diagnostics из-за раннего `kAltitudeRejected` exit.
Обе замечания подтверждены по текущему production call order и исправлены в этом плане.
...
3. **Добавить pure ground-plane geometry и единый per-beam decision API.**
   ...
   known-static classifier, выбирает минимальный positive expected range и множество всех
   candidates в фиксированном tie epsilon от этого минимума. Selection не должен зависеть от
   порядка вызова providers. До разрешения surface tie нужно проверить measured hit против
   **каждого** ближайшего tied candidate с tolerance именно этого provider:
...
4. **Зафиксировать update policy, не расширяя scope known-static поведения.**
   ...
   его скрыть, expected/ambiguous ground ничего не очищает и не занимает, invalid input не
   отключает lidar.
...
10. **Добавить portable automated tests и CMake target.**
    ...
    matching/behind/no-return measurement обязан дать tie ambiguity + `SuppressAll`. Расширить:
    ...
    - `current_lidar_overlay_test.cpp` — expected/ambiguous ground отсутствует, closer hit есть;
      ...
      status path и дают ground diagnostic без occupied overlay;
