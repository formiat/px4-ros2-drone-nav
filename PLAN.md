...
- Новый core-модуль `drone_city_nav/include/drone_city_nav/static_city_map.hpp`
  ...
  статической карты.
- Новый или расширенный core-модуль для merge-логики, например
  `drone_city_nav/include/drone_city_nav/grid_overlay.hpp`: union overlay
  источников с правилом, что occupied не может быть очищен другим источником.
...
- `drone_city_nav/tests/planner_core_test.cpp` и/или новый gtest:
  parser/rasterization/merge tests.
...
3. Добавить loader/rasterizer в core library.
   ...
   Parser должен явно валидировать version, positive resolution, positive sizes,
   finite numbers, duplicate ids и выход прямоугольников за bounds.

4. Реализовать union merge источников препятствий.

   Ввести helper уровня core, чтобы тестировать без ROS node:
   ...
   - `applyCurrentLidarOverlay(destination, scan)` остаётся node-level, но должен
     использовать то же правило: occupied wins.
...
9. Добавить debug publication для статической карты.
   ...
   Минимум: публиковать итоговый `/drone_city_nav/occupancy_grid`, как сейчас.
   Желательно: добавить transient topic `/drone_city_nav/static_map_grid`,
   чтобы в RViz можно было отдельно увидеть именно статическую карту. Это
   поможет отличить ошибку загрузки карты от ошибки lidar/memory.

10. Добавить unit tests.

   Тесты parser/rasterizer:
...
Основной риск лежит не в ROS plumbing, а в semantics объединения карт. Поэтому
сначала нужны маленькие core tests без Gazebo/PX4:

- parser/rasterization tests для `*.map2d`;
- merge precedence tests для static/memory/lidar;
- A* regression test: путь не проходит через static-only obstacle;
- A* regression test: memory free не очищает static obstacle.

После unit tests нужен один headless smoke-run с логами. Полный mission-run
дороже, поэтому его запускать после прохождения unit/quality, а не на каждую
мелкую правку.
