...
- Как соотносятся EGL warnings из GUI-прогона с предыдущими headless/server-render
  прогонами?
...
- В Gazebo log присутствуют `libEGL warning` и `egl: failed to create dri2
  screen`, но такие же warnings есть и в предыдущих headless/server-render
  прогонах; это system-level render-stack signal, не GUI-specific proof.
- `/pose/info` excerpt из GUI-прогона содержит pose entries для x500 mesh visual
  nodes (`base_link_visual`, `rotor_*_visual*`) и модели `x500_lidar_2d_0`.
- Тем же сохранённым grep не подтверждены yellow marker visuals
  (`yellow_drone_locator*`, `yellow_ground_projection*`).
...
- Поздний `/world/generated_city/pose/info` excerpt содержит pose entries для
  x500 mesh visual nodes (`base_link_visual`, `rotor_*_visual*`) и модели
  `x500_lidar_2d_0`:
  `log-host/investigate_render_gui_20260619_000134/gui_scene_queries.txt:567-639`.
- Сохранённый filtered `/world/generated_city/scene/info` query не дал matches
  между headings `scene info x500/visual excerpt` и `pose info x500 excerpt`:
  `log-host/investigate_render_gui_20260619_000134/gui_scene_queries.txt:34-35`.
...
2. Immediate technical cause: проблема не в отсутствии server-side модели.
   ...
   присутствуют, `/pose/info` публикует её положение и pose entries для x500
   mesh visual nodes.
...
5. Почему EGL/render stack остаётся релевантным, но не является
   GUI-specific proof: `libEGL warning` / `egl: failed to create dri2 screen`
   воспроизводятся и в GUI-прогоне, и в предыдущих headless/server-render
   прогонах.
6. Почему visual-scene boundary стал точнее: server pose topic содержит pose
   entries для x500 mesh visuals, но сохранённый `scene/info` grep не дал
   полезного x500 visual payload, а yellow marker visuals в сохранённом
   `gui_scene_queries.txt` не подтверждены.
7. Почему точный GUI-client root cause ещё не доказан: `scripts/run_city_mvp.sh`
   запускает `gz sim -g > /dev/null 2>&1`, поэтому stderr/stdout GUI-клиента
   теряются; screenshot из agent-сессии снять не удалось.
8. Почему code-level model layout всё ещё подозрителен: проект использует
...
| X500 mesh visual pose entries публикуются | `gui_scene_queries.txt:567-639`: `base_link_visual`, `rotor_*_visual*`, затем `x500_lidar_2d_0`. |
| Filtered `scene/info` не дал x500 visual matches | `gui_scene_queries.txt:34-35`: после heading `scene info x500/visual excerpt` сразу идёт `pose info x500 excerpt`. |
| GUI follow target доступен service layer-у | `gz_city_mvp.log:21-26`: первый timeout, затем 3 accepted attempts. |
| Render stack выдаёт warning не только в GUI | GUI `gz_city_mvp.log:9-18`, headless `gz_city_mvp.log:7-17`, transport `gz_city_mvp.log:7-17`: везде `libEGL warning` и `egl: failed to create dri2 screen`. |
...
- `gui_scene_queries.txt:567-639` содержит pose entries для x500 mesh visual
  nodes (`base_link_visual`, `rotor_*_visual*`) и модели `x500_lidar_2d_0`;
- сохранённый grep по `gui_scene_queries.txt` не подтверждает yellow marker
  visuals (`yellow_drone_locator*`, `yellow_ground_projection*`);
- filtered `scene/info` query не дал x500/visual matches между headings
  `scene info x500/visual excerpt` и `pose info x500 excerpt`;
- `libEGL warning` и `egl: failed to create dri2 screen` воспроизводятся и в
  GUI, и в headless/server-render прогонах;
...
- известно, что server pose topic содержит pose entries для x500 mesh visuals;
  неизвестно, получает ли GUI client эти entries и рендерит ли их;
- неизвестно, почему yellow marker visuals не появились в сохранённом
  `gui_scene_queries.txt` grep, хотя SDF expansion их содержит;
...
- render-stack/driver path как системный сигнал: `libEGL warning` /
  `egl: failed to create dri2 screen` воспроизводится в GUI и headless; это
  релевантно, но не является GUI-specific proof.
...
- EGL warning is a system-level render signal: low-medium as root-cause proof;
...
- `log-host/investigate_render_headless_20260618_233648/gz_city_mvp.log`
...
- `log-host/investigate_render_transport_20260618_234601/gz_city_mvp.log`
...
Filtered `scene/info` result is empty in the saved artifact: after
`scene info x500/visual excerpt`, the file immediately continues with
`pose info x500 excerpt`. This does not prove that scene visual entries do not
exist; it only means this bounded filtered command did not capture useful
x500/visual scene payload.

```text
log-host/investigate_render_gui_20260619_000134/gui_scene_queries.txt:567-639
1860:  name: "base_link_visual"
1962:  name: "rotor_0_visual"
1974:  name: "rotor_0_visual_motor_bell"
1984:  name: "rotor_1_visual"
1996:  name: "rotor_1_visual_motor_top"
2006:  name: "rotor_2_visual"
2018:  name: "rotor_2_visual_motor_top"
2028:  name: "rotor_3_visual"
2040:  name: "rotor_3_visual_motor_top"
2835:  name: "x500_lidar_2d_0"
```

The same saved grep did not find `yellow_drone_locator*` or
`yellow_ground_projection*` entries in `gui_scene_queries.txt`.
...
The same EGL warning pattern is also present in previous headless/server-render
runs:

```text
log-host/investigate_render_headless_20260618_233648/gz_city_mvp.log:7-17
libEGL warning ...
libEGL warning: egl: failed to create dri2 screen

log-host/investigate_render_transport_20260618_234601/gz_city_mvp.log:7-17
libEGL warning ...
libEGL warning: egl: failed to create dri2 screen
```
...
- `log-host/investigate_render_gui_20260619_000134/gui_scene_queries.txt:567-639`:
  pose entries for `base_link_visual`, `rotor_*_visual*` and
  `x500_lidar_2d_0`.
- `log-host/investigate_render_gui_20260619_000134/gui_scene_queries.txt:34-35`:
  filtered `scene/info` command produced no x500/visual matches before the
  saved `pose/info` excerpt.
- `log-host/investigate_render_gui_20260619_000134/gz_city_mvp.log:9-18`,
  `log-host/investigate_render_headless_20260618_233648/gz_city_mvp.log:7-17`,
  `log-host/investigate_render_transport_20260618_234601/gz_city_mvp.log:7-17`:
  EGL warnings reproduced in GUI and headless/server-render runs.
- `log-host/investigate_render_gui_20260619_000134/gz_city_mvp.log:21-26`:
...
2. High confidence: server-side модель имеет ожидаемые links и lidar/model
   topics.
3. High confidence: server pose topic содержит pose entries для x500 mesh
   visual nodes (`base_link_visual`, `rotor_*_visual*`) и модели
   `x500_lidar_2d_0`.
4. Medium-high confidence: сохранённый filtered `scene/info` query не дал
   x500/visual matches; это отрицательный результат конкретной команды, но не
   доказательство отсутствия scene visual entries вообще.
5. Medium-high confidence: yellow marker visuals не подтверждены сохранённым
   `gui_scene_queries.txt` grep, хотя SDF expansion их содержит.
6. High confidence: SDF валиден и раскрывается в модель с mesh visuals и yellow
   marker visuals.
7. High confidence: текущая проблема относится к boundary после server-side
   entity creation, а не к PX4 spawn, ROS planner/offboard или отсутствию модели.
8. Low-medium confidence as root-cause proof: EGL/render warning является
   системным render-stack signal, но не GUI-specific proof, потому что он
   воспроизводится и в GUI, и в headless/server-render runs.
9. Medium confidence: `bbcc6a3` является code-level suspect для marker
   regression, потому что перенёс visual locator внутрь merged lidar child link.
10. Medium confidence: это не полностью объясняет отсутствие base x500 mesh
   visuals; значит точная render-причина остаётся unresolved boundary.
11. High confidence: `FAIL: planner publishes a path` в старом `run.out` является
   отдельным validator mismatch: текущий log format изменился, а regex остался
   старым. ROS log содержит реальные `Published path`.
12. High confidence: текущий launcher недостаточно логирует GUI-render проблемы,
...
- `log-host/investigate_render_gui_20260619_000134`
  - GUI-прогон с transport evidence и воспроизведёнными EGL warnings.
...
## H2: локальный EGL/render backend влияет на отображение

Confidence: low-medium.

Immediate cause: Gazebo render path получает EGL warnings.
...
- из-за `gz sim -g > /dev/null 2>&1` неизвестно, какие warnings/errors были
  именно у GUI client;
- EGL warnings воспроизводятся и в headless/server-render runs, поэтому это
  system-level render-stack signal, а не GUI-specific proof.
...
- Server pose topic содержит pose entries для x500 mesh visual nodes
  (`base_link_visual`, `rotor_*_visual*`) и модели `x500_lidar_2d_0`.
- Yellow marker visual entries сохранённым `gui_scene_queries.txt` grep не
  подтверждены, хотя SDF expansion их содержит.
...
- EGL warning воспроизводится и в GUI, и в headless/server-render path; это
  системный render-stack signal, но не GUI-specific proof.
...
- Если GUI logs чистые, но `pose/info` продолжает содержать x500 mesh visual
  pose entries, нужно проверять именно GUI client receive/render stage, а не
  server-side spawn.
...
- Относятся ли текущие `libEGL warning` к GUI client, server render sensor path
  или обоим?
- Server pose topic содержит visual pose entries для x500 mesh, но получает ли
  GUI client эти entries и рендерит ли их?
- Почему filtered `scene/info` query не дал полезного x500 visual payload:
  timing, grep scope, topic semantics или реальное отсутствие scene payload?
- Почему yellow marker visuals не подтверждены сохранённым `gui_scene_queries.txt`
  grep, хотя SDF expansion их содержит?
