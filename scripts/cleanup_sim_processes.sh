...
container_has_simulation_processes() {
  ...
  grep -Eiq \
    'gz[[:space:]]+sim|MicroXRCEAgent|PX4-Autopilot|px4_sitl|ros2[[:space:]]+launch[[:space:]]+drone_city_nav[[:space:]]+city_nav\.launch\.py|run_drone_nav_sim\.sh|make[",[:space:]]+sim-(gui|headless)|sim-(gui|headless)|rviz2.*city_nav_debug(_top_down)?\.rviz|/drone_city_nav/(collision_crash_node|lidar_debug_node|mission_monitor_node|obstacle_memory_node|planner_node|px4_offboard_node)' \
    <<< "${combined_text}"
}
