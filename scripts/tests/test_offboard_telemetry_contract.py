...
class OffboardTelemetryContractTest(unittest.TestCase):
    ...
    def test_telemetry_logs_path_command_and_obstacle_diagnostics(self) -> None:
        ...
        self.assertIn("cross_track_speed_factor=%.2f", self.offboard_text)
        self.assertIn("cross_track_limited_speed=%.2f", self.offboard_text)
    ...
    def test_telemetry_writes_jsonl_flight_blackbox(self) -> None:
        ...
        self.assertIn("cross_track_speed_factor", self.offboard_text)
        self.assertIn("cross_track_limited_speed_mps", self.offboard_text)
    ...
    def test_runtime_configs_expose_attitude_topic(self) -> None:
        for relative_path in ("drone_city_nav/config/urban_mvp.yaml",):
            with self.subTest(relative_path=relative_path):
                ...
                self.assertIn("cross_track_speed_guard_start_m: 2.0", text)
                self.assertIn("cross_track_speed_guard_full_m: 6.0", text)
                self.assertIn("cross_track_speed_guard_min_factor: 0.35", text)
