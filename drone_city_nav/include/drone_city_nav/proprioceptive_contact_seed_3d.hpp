...
// suppresses it for body positions that come no closer to it than the seed.
//
...
// The same seed for a validation against one observed world; the tolerance is
// that world's voxel quantization. Returns no seed without an observed world:
// a static world is not evidence the vehicle observed from where it stands.
...
proprioceptiveContactSeed3D(const Point3& position,
                            ...
                            const ObservedOccupancyGrid3D* observed_occupancy) noexcept;
