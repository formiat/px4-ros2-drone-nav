...
enum class CooperativePassageRouteStatus : std::uint8_t {
  kCentered,
  kApplied,
...
struct CooperativePassageAssignment {
  ...
  CooperativePassageRouteStatus status{CooperativePassageRouteStatus::kCentered};

  [[nodiscard]] bool applied() const noexcept {
    return status == CooperativePassageRouteStatus::kApplied;
  }
...
struct CooperativePassageRouteConfig {
...
struct CooperativePassageRouteResult {
  std::vector<RouteSample3D> route;
  std::vector<ConstrainedRouteSpan> constrained_spans;
  std::vector<CooperativePassageAssignment> assignments;
  std::size_t applied_offset_count{0U};
  bool valid{false};
};

[[nodiscard]] CooperativePassageRouteResult applyCooperativePassageCorridors(
    ...
    const CooperativePassageRouteConfig& config);

[[nodiscard]] const char*
cooperativePassageRouteStatusName(CooperativePassageRouteStatus status) noexcept;
