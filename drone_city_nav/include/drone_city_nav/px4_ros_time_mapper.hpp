...
enum class Px4RosTimeObservationStatus : std::uint8_t {
  kAccepted,
  kRejectedInvalid,
  kRejectedDiscontinuity,
  kRebased,
};

struct Px4RosTimeObservation {
  Px4RosTimeObservationStatus status{Px4RosTimeObservationStatus::kRejectedInvalid};
  std::uint64_t generation{0U};

  [[nodiscard]] bool rebased() const noexcept {
    return status == Px4RosTimeObservationStatus::kRebased;
  }
};
...
public:
  ...
  Px4RosTimeObservation observeTimesync(std::uint64_t adjusted_timestamp_us,
...
private:
  ...
  [[nodiscard]] bool stepsConsistent(const Sample& previous,
                                     const Sample& current) const noexcept;
  void acceptSample(const Sample& sample);
  [[nodiscard]] bool stageRebaseCandidate(const Sample& sample);
  void rebaseFromCandidates();
  void refit() noexcept;

  Px4RosTimeMapperConfig config_{};
  std::deque<Sample> samples_;
  std::deque<Sample> rebase_candidates_;
