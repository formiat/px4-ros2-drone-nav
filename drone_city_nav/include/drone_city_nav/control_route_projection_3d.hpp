...
#define DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE __host__ __device__
#else
#define DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE
#endif

struct ControlRouteProjection3D {
...
[[nodiscard]] DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE inline float
creditedControlRouteProgressM3D(const float projected_station_m,
                                const float initial_station_m,
...
[[nodiscard]] DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE inline float
controlRouteTrackingSpeedMps3D(const MotionState3D& state,
                               const ControlRouteProjection3D& projection) noexcept {
...
[[nodiscard]] DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE inline ControlRouteProjection3D
projectOntoControlRoute3D(const MotionState3D& state,
                          const ControlRouteSample3D* route_points,
                          const std::size_t route_point_count,
                          const float minimum_station_m) noexcept {
  ControlRouteProjection3D result;
  float best_squared_distance = FLT_MAX;
  for (std::size_t index = 0U; index + 1U < route_point_count; ++index) {
    const ControlRouteSample3D first = route_points[index];
    const ControlRouteSample3D second = route_points[index + 1U];
...
[[nodiscard]] inline ControlRouteProjection3D
projectOntoControlRoute3D(const MotionState3D& state,
                          const std::span<const ControlRouteSample3D> route,
                          const float minimum_station_m) noexcept {
  return projectOntoControlRoute3D(state, route.data(), route.size(),
                                   minimum_station_m);
}

#undef DRONE_CITY_NAV_CONTROL_ROUTE_HOST_DEVICE
