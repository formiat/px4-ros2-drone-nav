"""Composable diagnostics helpers for multi-vehicle missions."""
...
def make_selected_diagnostics_components(
    ...
    reselection_policy,
    reselection_delay_s,
    gazebo_aligned_rviz_axes_swapped=True,
):
    spectator = ComposableNode(
        package="drone_city_nav",
        plugin="drone_city_nav::MultiVehicleSpectatorNode",
        name="multi_vehicle_spectator_node",
        parameters=[
            {
                "use_sim_time": True,
                ...
                "gazebo_aligned_rviz_axes_swapped": gazebo_aligned_rviz_axes_swapped,
            }
        ],
        extra_arguments=[{"use_intra_process_comms": True}],
    )
    mux = ComposableNode(
        package="drone_city_nav",
        plugin="drone_city_nav::MultiVehicleDiagnosticsMuxNode",
        name="multi_vehicle_diagnostics_mux_node",
        parameters=[
            {
                "use_sim_time": True,
                "vehicle_ids": vehicle_ids,
...
def make_diagnostics_container(components, cpu_prefix):
    return ComposableNodeContainer(
        ...
        namespace="",
        name="multi_vehicle_diagnostics_container",
        output="screen",
        prefix=cpu_prefix,
        parameters=[
            {
