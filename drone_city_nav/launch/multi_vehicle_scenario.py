#!/usr/bin/env python3
"""Load and validate a canonical cooperative multi-vehicle scenario."""
...
def load_multi_vehicle_scenario(
    ...
    gazebo_world_name = world.get("gazebo_world_name")
    if (
    ...
    if len(vehicles) < 2:
        raise ValueError(
            "cooperative traffic scenario must contain at least two vehicles"
        )
    ...
    mission_name = document.get("mission_name", "cooperative_traffic")
    ...
    return {
        ...
        "vehicles": vehicles,
        "civilian_ids": [vehicle["id"] for vehicle in vehicles],
