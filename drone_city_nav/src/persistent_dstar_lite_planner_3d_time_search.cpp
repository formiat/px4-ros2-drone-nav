...
void PersistentDStarLitePlanner3DImpl::initializeExecutionTimeSearch(
  ...
  execution_time_refiner_.initialized_ = true;
  execution_time_refiner_.start_ = start_state;
  execution_time_refiner_.start_from_rest_ =
      std::hypot(std::hypot(request.velocity.x, request.velocity.y),
                 request.velocity.z) <= kDirectionEpsilon;
  execution_time_refiner_.costs_.emplace(start_state, 0.0);
  execution_time_refiner_.open_.push(PersistentPlannerTimeQueueEntry3D{
      .estimated_total_s = executionTimeHeuristic(start_state),
      .cost_from_start_s = 0.0,
      .state = start_state,
      .sequence = ++execution_time_refiner_.queue_sequence_,
  });
...
void PersistentDStarLitePlanner3DImpl::seedExecutionTimeIncumbent() {
  ...
  PersistentPlannerTimeState3D current = execution_time_refiner_.start_;
  double cost = 0.0;
  ...
  execution_time_refiner_.spatial_incumbent_ = spatial_path;
  execution_time_refiner_.goal_cost_s_ = cost;
}

bool PersistentDStarLitePlanner3DImpl::hasExecutionTimeIncumbent() const noexcept {
  return execution_time_refiner_.goal_.has_value() ||
         !execution_time_refiner_.spatial_incumbent_.empty();
}

std::vector<Point3> PersistentDStarLitePlanner3DImpl::bestExecutionTimePath() {
  if (execution_time_refiner_.goal_.has_value()) {
    return extractExecutionTimePath();
  }
  std::vector<Point3> path = execution_time_refiner_.spatial_incumbent_;
  if (path.empty()) {
    return path;
  }
  ...
  PersistentPlannerNode3D previous = execution_time_refiner_.start_.position;
...
double PersistentDStarLitePlanner3DImpl::executionTimeHeuristic(
  ...
  if (!dstar_session_.cost_to_goal_heuristic_admissible_) {
    return euclidean;
  }
  const auto record = dstar_session_.records_.find(state.position);
  return record != dstar_session_.records_.end() && std::isfinite(record->second.g)
...
double PersistentDStarLitePlanner3DImpl::executionTimeTransitionCost(
  ...
  if (first.incoming.empty()) {
    if (first == execution_time_refiner_.start_ &&
        execution_time_refiner_.start_from_rest_) {
      return translation_cost +
...
PersistentDStarLitePlanner3DImpl::continueExecutionTimeSearch(
  ...
  if (execution_time_refiner_.complete_) {
  ...
  while (true) {
    while (!execution_time_refiner_.open_.empty()) {
      const PersistentPlannerTimeQueueEntry3D& entry =
          execution_time_refiner_.open_.top();
      const auto found = execution_time_refiner_.costs_.find(entry.state);
      if (found != execution_time_refiner_.costs_.end() &&
          !costLess(found->second, entry.cost_from_start_s) &&
          !costLess(entry.cost_from_start_s, found->second)) {
        break;
      }
      execution_time_refiner_.open_.pop();
    }

    if (execution_time_refiner_.open_.empty() ||
        (hasExecutionTimeIncumbent() &&
         !costLess(execution_time_refiner_.open_.top().estimated_total_s,
                   execution_time_refiner_.goal_cost_s_))) {
      execution_time_refiner_.complete_ = true;
      return hasExecutionTimeIncumbent()
                 ? std::optional<std::vector<Point3>>{bestExecutionTimePath()}
                 : std::nullopt;
    }
    ...
    const PersistentPlannerTimeQueueEntry3D current =
        execution_time_refiner_.open_.top();
    execution_time_refiner_.open_.pop();
    ++expansions;
    if (current.state.position == goal_) {
      ...
      if (std::isfinite(total_cost) &&
          (!hasExecutionTimeIncumbent() ||
           costLess(total_cost, execution_time_refiner_.goal_cost_s_))) {
        execution_time_refiner_.goal_ = current.state;
        execution_time_refiner_.goal_cost_s_ = total_cost;
      }
      continue;
    }
    ...
    for (const PersistentPlannerNode3D successor_position :
      ...
      const auto existing = execution_time_refiner_.costs_.find(successor);
      if (existing != execution_time_refiner_.costs_.end() &&
          !costLess(candidate_cost, existing->second)) {
        continue;
      }
      execution_time_refiner_.costs_[successor] = candidate_cost;
      execution_time_refiner_.parents_[successor] = current.state;
      ++execution_time_refiner_.queue_sequence_;
      if (execution_time_refiner_.queue_sequence_ == 0U) {
        execution_time_refiner_.queue_sequence_ = 1U;
      }
      const double estimated_total = candidate_cost + executionTimeHeuristic(successor);
      if (hasExecutionTimeIncumbent() &&
          !costLess(estimated_total, execution_time_refiner_.goal_cost_s_)) {
        continue;
      }
      execution_time_refiner_.open_.push(PersistentPlannerTimeQueueEntry3D{
...
std::vector<Point3> PersistentDStarLitePlanner3DImpl::extractExecutionTimePath() {
  ...
  PersistentPlannerTimeState3D current = *execution_time_refiner_.goal_;
  while (true) {
    states.push_back(current);
    if (current == execution_time_refiner_.start_) {
      break;
    }
    if (states.size() >= config_.maximum_extracted_path_nodes) {
      return {};
    }
    const auto parent = execution_time_refiner_.parents_.find(current);
    if (parent == execution_time_refiner_.parents_.end()) {
      return {};
    }
