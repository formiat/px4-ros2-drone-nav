...
class MultiVehicleSpectatorNode final : public rclcpp::Node {
public:
  explicit MultiVehicleSpectatorNode(const rclcpp::NodeOptions& options)
      : Node{"multi_vehicle_spectator_node", options},
    ...
    for (const std::int64_t role : role_values) {
      if (role != msg::VehicleDestroyed::ROLE_CIVILIAN) {
...
makeMultiVehicleSpectatorNode(const rclcpp::NodeOptions& options) {
  return std::make_shared<MultiVehicleSpectatorNode>(options);
}

} // namespace drone_city_nav

RCLCPP_COMPONENTS_REGISTER_NODE(drone_city_nav::MultiVehicleSpectatorNode)
