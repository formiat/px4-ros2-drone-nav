...
struct IncrementalTopologyGraph3D::Impl {
  ...
  rebuild(const Occupancy& occupancy, std::uint64_t update_revision,
          std::vector<IncrementalTopologyBlockIndex3D> rebuilt_blocks,
          std::size_t requested_dirty_chunks, bool full_reset,
          std::optional<std::chrono::steady_clock::time_point> deadline = std::nullopt,
          std::vector<IncrementalTopologyBlockIndex3D>* deferred_blocks = nullptr);
...
#include "incremental_topology_graph_3d_rebuild_impl.hpp"
