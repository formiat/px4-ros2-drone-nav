...
smoothTrajectoryTurns(const std::span<const TrajectoryPointSample> samples,
  ...
  for (std::size_t pass = 0U; pass < max_passes; ++pass) {
    ...
    const std::vector<CornerCandidate> corners = cornerCandidatesBySeverity(
        result.samples, config, speed_config, detection_stats);
    ...
    SmoothingAttempt accepted_attempt{};
    bool has_accepted_attempt = false;
    std::size_t selected_candidate_diagnostic_index = 0U;
    bool has_selected_candidate_diagnostic_index = false;
    double accepted_corner_s_m = std::numeric_limits<double>::quiet_NaN();

    for (const CornerCandidate& corner : corners) {
      ...
      if (has_accepted_attempt) {
        ...
        accepted_corner_s_m = corner_s_m;
        break;
      }
      ...
      if (rejected_attempt.has_value()) {
        const SmoothingAttempt& rejected = rejected_attempt.value();
        result.stats.corner_diagnostics.push_back(
            cornerDiagnosticFromAttempt(rejected, corner_s_m));
      }
    }
    if (!has_accepted_attempt) {
      break;
    ...
    if (has_selected_candidate_diagnostic_index &&
        selected_candidate_diagnostic_index <
            result.stats.candidate_diagnostics.size()) {
      const std::size_t diagnostic_index = selected_candidate_diagnostic_index;
      result.stats.candidate_diagnostics[diagnostic_index].decision = "selected";
      updateCandidateSpeedDiagnostics(
          result.stats.candidate_diagnostics[diagnostic_index], accepted_attempt);
    }
    result.stats.corner_diagnostics.push_back(
        cornerDiagnosticFromAttempt(accepted_attempt, accepted_corner_s_m));
