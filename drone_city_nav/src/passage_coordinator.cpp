...
PassageCoordinator::update(const PassageCoordinatorInput& input) {
  ...
  const double capture_min_z = safe_min_z + config_.vertical_capture_hysteresis_m;
  const double capture_max_z = safe_max_z - config_.vertical_capture_hysteresis_m;
  ...
  const bool captured = inside_capture_window && capture_velocity_safe;
  ...
  if (vertical_alignment_active_) {
    if (captured) {
      ...
      traversal_latched_ = true;
             ...
             distance_to_entry_m <=
                 std::max(config_.minimum_stationary_trigger_distance_m,
                          required_distance_m)) {
    vertical_alignment_active_ = true;
    hold_position_ = Point2{input.state.x, input.state.y};
  } else if (!partial_from_inside_ && !traversal_latched_ &&
             coordinates.longitudinal_m >= -half_depth_m && captured) {
    traversal_latched_ = true;
  }
  ...
  if (traversal_latched_ && !partial_from_inside_ && !inside_retention_window) {
    vertical_alignment_active_ = true;
  ...
  PassageCoordinatorResult result{
      ...
      .vertical_ready =
          partial_from_inside_ || (traversal_latched_ && inside_retention_window),
