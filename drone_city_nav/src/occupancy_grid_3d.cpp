...
constexpr std::uint32_t kLegacyRawOnlyVersion{4U};
...
OccupancyGrid3D OccupancyGrid3D::load(const std::filesystem::path& path) {
  ...
  if ((version != kVersion && version != kLegacyRawOnlyVersion) ||
  ...
  if (version == kLegacyRawOnlyVersion) {
    const std::uint32_t embedded_region_count =
        readValue<std::uint32_t>(stream, "legacy embedded topology region count");
    const std::uint32_t embedded_traversal_count =
        readValue<std::uint32_t>(stream, "legacy embedded topology traversal count");
    if (embedded_region_count != 0U || embedded_traversal_count != 0U) {
      throw std::runtime_error{
          "legacy Occupancy3D contains embedded topology; regenerate separate "
          "Occupancy3D and FreeSpaceTopology3D artifacts"};
    }
  }
