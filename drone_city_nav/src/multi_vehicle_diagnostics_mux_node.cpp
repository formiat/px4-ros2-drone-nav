...
class MultiVehicleDiagnosticsMuxNode final : public rclcpp::Node {
public:
  explicit MultiVehicleDiagnosticsMuxNode(const rclcpp::NodeOptions& options)
      : Node{"multi_vehicle_diagnostics_mux_node", options} {
    frame_id_ = declare_parameter<std::string>("frame_id", "map");
    vehicle_ids_ = declare_parameter<std::vector<std::string>>(
        "vehicle_ids", {"civilian_0", "civilian_1", "civilian_2"});
    validateVehicleIds();
    ...
    direction_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
        declare_parameter<std::string>("direction_marker_topic",
                                       "/drone_city_nav/vehicle_directions"),
        reliable_qos);
    ...
    RCLCPP_INFO(get_logger(),
                "MULTI_VEHICLE_DIAGNOSTICS_MUX_READY vehicles=%zu frame_id='%s'",
...
private:
  ...
  void onControlFeedback(const msg::MppiControlFeedback& feedback,
    ...
    if (!assessment.valid()) {
      ...
      RCLCPP_WARN_THROTTLE(
          ...
          "MULTI_VEHICLE_DIAGNOSTICS_CONTROL_FEEDBACK rejected=true vehicle_id='%s' "
  ...
  void onExecutionHorizon(const msg::MppiTrajectoryHorizon& horizon,
    ...
    if (!admission.accept_identity &&
      ...
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 2000,
          "MULTI_VEHICLE_DIAGNOSTICS_HORIZON rejected=true vehicle_id='%s' "
    ...
    if (revoked) {
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000,
                           "MULTI_VEHICLE_DIAGNOSTICS_HORIZON revoked=true "
  ...
  void onSpectatorTarget(const msg::SpectatorTarget& target) {
    ...
    if (iterator == vehicle_ids_.end()) {
      RCLCPP_WARN(
          get_logger(),
          "MULTI_VEHICLE_DIAGNOSTICS_SELECTION rejected=true reason=unknown_vehicle "
          "vehicle_id='%s'",
    ...
    RCLCPP_INFO(get_logger(),
                "MULTI_VEHICLE_DIAGNOSTICS_SELECTION vehicle_id='%s' index=%zu "
  ...
  void publishDirections() {
    ...
    for (std::size_t index = 0; index < vehicle_ids_.size(); ++index) {
      ...
      marker.ns = "vehicle_direction_" + vehicle_ids_[index];
...
makeMultiVehicleDiagnosticsMuxNode(const rclcpp::NodeOptions& options) {
  return std::make_shared<MultiVehicleDiagnosticsMuxNode>(options);
}

} // namespace drone_city_nav

RCLCPP_COMPONENTS_REGISTER_NODE(drone_city_nav::MultiVehicleDiagnosticsMuxNode)
