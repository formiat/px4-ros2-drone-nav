...
void ProductionMppiNode::processStaticEsdf3D() {
  if (!use_static_map_) {
    return;
  }
  ...
  if (use_static_map_) {
    ...
    const WorldPipelineResidentSnapshot3D active = world_pipeline_->residentSnapshot();
    const std::shared_ptr<const WorldSnapshot3D>& active_world = active.world;
    const ProductionWorldBuildTelemetry3D& active_world_build = active.telemetry;
    const bool roi_refresh_pending = static_roi_refresh_lifecycle_.pending(roi_refresh);
    ...
    ProductionWorldBuildTelemetry3D world_build =
        proactive_roi_refresh ? active_world_build : ProductionWorldBuildTelemetry3D{};
    WorldSnapshot3D world;
    world.producer_instance_id = active_world ? active_world->producer_instance_id : 0U;
    ...
    const std::optional<LocalWorldGeneration> local_world_generation =
        publication->issueGeneration(static_world_version,
                                     activation_navigation.revision, world.revision,
                                     upload.revision);
    ...
    const bool coherent_generation = publication->publish(world_snapshot, world_build);
    publication.reset();
