...
ShadowMppiNode::ShadowMppiNode()
    : Node{"shadow_mppi_node"} {
  ...
  diagnostics_output_dir_ =
      declare_parameter<std::string>("diagnostics_output_dir", "log/shadow_mppi");
  ...
  if (!enabled_) {
    RCLCPP_INFO(get_logger(), "Shadow MPPI disabled");
    return;
  }
  if (!(tick_rate_hz_ > 0.0) || !(deadline_ms_ > 0.0)) {
    throw std::invalid_argument{"invalid shadow MPPI timing configuration"};
  }
  ...
  diagnostics_stream_.open(diagnostics_output_dir_ / "shadow_mppi_ticks.jsonl",
                           std::ios::app);
  ...
  active_path_sub_ = create_subscription<nav_msgs::msg::Path>(
      declare_parameter<std::string>("active_path_topic",
                                     "/drone_city_nav/final_trajectory_path"),
      rclcpp::QoS{1}.reliable(), [this](nav_msgs::msg::Path::ConstSharedPtr message) {
        onActivePath(std::move(message));
      });
  path_pub_ = create_publisher<nav_msgs::msg::Path>(
      declare_parameter<std::string>("shadow_path_topic",
                                     "/drone_city_nav/shadow_mppi/path"),
      rclcpp::QoS{1}.best_effort());
  markers_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
      declare_parameter<std::string>("shadow_markers_topic",
                                     "/drone_city_nav/shadow_mppi/markers"),
      rclcpp::QoS{1}.best_effort());
  status_pub_ = create_publisher<std_msgs::msg::String>(
      declare_parameter<std::string>("shadow_status_topic",
                                     "/drone_city_nav/shadow_mppi/status"),
  ...
  RCLCPP_INFO(get_logger(),
              "Shadow MPPI ready: rollouts=%zu steps=%zu rate=%.1fHz deadline=%.1fms "
              "offboard_output=none known_solids=%zu",
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::spin(std::make_shared<drone_city_nav::ShadowMppiNode>());
  rclcpp::shutdown();
  return 0;
}
