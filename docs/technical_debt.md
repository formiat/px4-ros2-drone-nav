...
| The 0.35 m cross-track reference | A reference per localization profile: 0.35 m on GNSS (measured), 1.0 m on the odometry profiles, half the capture radius the note guards. | done, 81e158ad |
...
| The hover's rest clearance | The clearance goes from 0.27 to 0.35 m, the autopilot's rest error at p95 plus the estimate's slide at p95 (0.15 + 0.14 on the camera profile), the only direction the invariant allows. | done, 173155d6, accepted with both series (cameras r644 to r648, lidar r650 to r654) |
| The recovery at 2 m/s^2 | Left as the cubic connector's price: 0.9 to 1.0 s a flight, under 1 percent, is not provable by five flights and not worth a series pair; the guide seed for straight non-terminal intervals (3326013f) stays reverted. The entry leaves the register; its history is in the speed goal's and the investigations goal's journals. | done, the entry removed |
| The braking laws' flicker | Investigated on the records flip by flip, then the one mechanism the records named repaired: the reference's climb after a fall starts from the vehicle's actual speed (ccf6bd6d), the falls untouched. The falls themselves remain, see the entry. | done in part, ccf6bd6d, accepted with both series on 173155d6 |
...
| L4 a reference visual-inertial system | no | medium: without one, nobody knows how good the estimator is | low | low | decided, see above; with L1, after item 19's measurement | M | the container image; a tool, not a criterion |
| L5 the IMU over a best-effort transport | no | medium: a real vehicle has another link | medium: the uXRCE transport and its QoS | low in simulation on a quiet host: 12 to 16 ms at most (r632 to r642) | measured 2026-09-25; on the real link a reliable stream or a higher rate; nothing in the simulator | M | the estimators, both series; holes under 0.1 s under any load |
...
| S3 the pair beside the lidar below real time | no | low: not a target configuration | low | low | roadmap item 20 stages 0 and 1: the loop on the simulation clock, the factor a parameter, this configuration at 0.5 | none | comparability of the figures only |
| S4 the braking laws' flicker | no | medium: 5 percent of a camera flight, 14 percent of a lidar flight | low for the return (done); the falls are the planner's and the clearance laws' | low: slow but safe | the return repaired (ccf6bd6d, both series on 173155d6); the falls with P5 and P1 | done in part | the speed laws; the mean speed unchanged on cameras, +1.8 percent on the lidar |
...
| C2 the horizon's lateral deviations into unknown space | no | medium | medium: a tube about the route for the horizon | low: the validators hold the speed | measured in roadmap item 17 stage 4 (the ring's lateral range); the remainder decided on the number | L | the controller and the tube; the effect on speed unmeasured |
...
| C5 the nearest pass closer on the vision memory | no | none: the figure was five camera flights against one lidar flight on a 0.5 m grid; on ten against ten at 0.25 m the profiles are alike (closest pass median 0.77 against 0.76 m) | none | none | measured 2026-09-25; left the register, the one sub-body approach (r614, 0.48 m, a shaft) is the drift's, with L1 | done | none |
...
| P2 the tick past its 20 ms, the planner spending its whole budget | no | low: it works | high: the tick's architecture | low | re-measured in roadmap item 20 stage 0 (tick and planner percentiles on the node clock); the rework stays its own | XL | all of execution |
| P3 the hover drifts past the rest clearance | no | medium | none | none | done: 0.35 m over the hull at the rest pose (173155d6), both series, rest rejections 0 to 3 per flight as before | done | none |
...
| P5 the planner's stages unscheduled, the livelock closed by the probe deadline only | medium | high: the first requirement (a mission incomplete) | high: the planner | medium: a rare trigger, none in twenty flights after the repair | roadmap item 18 stage 0, first step: a deadline on every stage before the memory decays; stall recovery as the lever | L | the planner, both series |
| P6 the recovery at 2 m/s^2 against the 4 admitted | no | low: 0.9 to 1.0 s per flight, under 1 percent | none | none | decided, see above: left as the connector's price, the entry removed | done | none |
...
| P9 the onboard loop on the wall clock, the world on the simulation clock | no at real time; yes for a slowed run | medium: a slow simulation flatters the stack, a slowed one does not lighten the host | medium: startup waits for `/clock`, timer jitter of one physics step | low at real time | planned, roadmap item 20 stage 0: the two ticks on the node clock, an audit of the other timers and watchdogs, both series at a factor of 1.0 | M | the planning and offboard ticks, tick percentiles, item 15's note |
| R1 the 2D lidar path still in the tree | no | low | medium: the base model and the airframe change under every flight | low | decided, see above; lands in roadmap item 11 with the 3D static maps | L | models, launches, scripts, scenarios, checks, pages; both series |
...
| N1 multi-vehicle missions on the camera defaults | no | high for item 15 | none | medium: item 15 may uncover defects | roadmap item 15 flies two vehicles on the lidar profile by its own decision; the camera defaults for several vehicles stay unflown, by design | series | the cooperative missions |
| N2 the cave and the finals locations | no | medium | none | medium: location-independent code unproved | roadmap item 11: the survey acquisition path flies every reachable part of a new environment | series | none |
| N3 one start, one goal | no | medium | none | medium | roadmap item 19 flies the mission back (a second goal, the other direction) and a goal outside the map; other starts and goals stay a series of their own | series | none |
...
Repaired since, on the owner's "all together" of the same day: L3, S2, R2
(three rules, r600, r629, r633), P7 and P4, the last two accepted with both
series on a34690ba (cameras r632 to r637 without r635: 1.75 to 1.90 m/s on
the simulation clock, mean 1.80; lidar r638 to r642: 2.41 to 2.82, mean
2.62). R1 stayed: its premise failed. The entries below no longer carry
them.

On 2026-09-26, P3 and the return of S4 (ccf6bd6d, 173155d6), accepted with
both series on 173155d6: cameras r644 to r648 1.73 to 1.87 m/s on the
simulation clock, mean 1.79; lidar r650 to r654 2.51 to 2.88, mean 2.67;
the truth at the goal 0.24 to 1.28 m, no crash, no contact.
...
|---|---|---|---|
| The visual-inertial estimate drifts without bound: an odometry with no loop closure. The goal-in-truth check leaves 0.6 m of margin on the worst accepted flight, and a mission several times longer will exceed the 2.0 m capture radius. The remedy is long-lived points in the filter's state or relocalization against a map. The closest approaches to the true occupancy are this drift's too (investigations goal 2026-09-25, margin_truth.py on a 0.25 m truth grid): at the ticks where the stack believed its body at the prohibited margin, the true distance from the centre to the nearest occupied voxel's centre was p05 0.81 / p50 1.32 m on the ten camera flights and 0.85 / 1.56 on the ten lidar flights, min 0.58 m (r634) and 0.48 m (r614, in the shaft at (56, -25), 7 percent of that flight's margin ticks below the body radius): the wall was mapped where the estimate stood, the vehicle drifted, and the memory kept the wall. The two profiles' closest passes are alike (median 0.77 against 0.76 m, min 0.50 against 0.48). Measured next, not reworked, in two roadmap items: item 19 flies the mission out and back, a path twice as long, and judges the arrival at the start by the capture radius, which is where this drift first meets a requirement; item 15 stage 4 measures the frame error between two vehicles, which is two of these drifts. The rework stays its own. | 0.1 to 0.4 percent of the path; the true position 0.47 to 1.42 m from the goal at its acknowledgement (r579 to r583), 1.97 and 2.31 m before the gyroscope noise was corrected (r572, r575) | (b) | item 16 |
...
| No reference visual-inertial system has been run on the recorded flights. OpenVINS without ROS needs Ceres, which builds from source in the container, and Boost.Filesystem, which the container image does not carry, and then a runner for these records. It is a tool and no criterion; adding the package to the image is a decision about the image. Decided 2026-09-25: not now; a tool image when the drift rework starts. Starts with the drift rework, whose measurements item 19 delivers. | attempted 2026-09-20, stopped at configuration | (c) | item 16 |
| The autopilot's IMU reaches the estimators over a best-effort transport at 83 to 92 Hz and loses bursts when the host stalls. The filter grows its uncertainty over a hole and recovers; a reliable or higher-rate IMU path is a change of the autopilot's transport configuration. Measured again on the twenty acceptance flights of 2026-09-25 (r632 to r642, a quiet host): the largest gap either estimator saw is 12 to 16 ms, one sample period, and the lag 0; the hole exists only under a stalled host. No repair in the simulator; on a real vehicle the link is another one and the stream's reliability or rate is set there. | holes of 0.33 to 0.69 s under a stalled host (r547, r549), at most 0.1 s otherwise (r550); 12 to 16 ms at most on a quiet host (r632 to r642) | (b) | item 16 |
...
| The heaviest configuration, the stereo pair rendered beside the lidar, does not hold real time on the reference workstation, so its speeds are not comparable with either sensor set's. Lands in roadmap item 20: with the onboard loop on the simulation clock (stage 0) and the factor a parameter of the run (stage 1), this configuration flies at 0.5 with the loop slowed with it, and its figures become comparable with a real-time series by the rule item 20 states. | real-time factor 0.83; 1.9 to 2.4 m/s (r566 to r570) | (a) | item 16 |
| The braking laws flicker: the reference falls by more than 1 m/s and returns within 2 s 32 to 52 times per camera flight (p50 0.35 s long; the sensor-braking law at the bottom of 60 to 75 percent of them) and 25 to 34 times per lidar flight (p50 0.78 s; the clearance and route-clearance laws). Flip by flip (r632 to r642): two thirds of the camera flips and their price are route replacement transients (a no-route gap of 0.1 to 0.2 s, 14 to 26 per flight; a fresh route whose first tick reads a law low, 6 to 8 percent of the 150 to 180 replacements a flight; a route replaced twice within a second), the rest the clearance laws at walls; on the lidar half of them are the clearance laws. The return was the price: the reference climbed from its fallen level at 4 m/s^2 while the vehicle still flew, sat rise-limited below the actual speed for 10.3 s per camera flight and 14.8 s per lidar flight, and the route-directed seed braked the vehicle toward it. ccf6bd6d starts the climb from the actual speed: that time is 0.3 to 1.9 s on the camera series r644 to r648 and 1.5 to 6.0 s on the lidar series r650 to r654, the speed lost per flip p50 0.12 to 0.02-0.11 m/s, the flips' count unchanged; the mean speed did not move on cameras (1.791 against 1.798) and rose 1.8 percent on the lidar (2.665 against 2.617), inside the spread, because after a flip the recovery is bounded by the horizon's own law (the connector's (4 / 3)(reference - speed) on straight intervals), not by the reference. What remains is the falls: the route churn (a route every 0.4 to 0.5 s on cameras, half of them shorter than 0.5 s) and the no-route gaps are the planner's (P5), the walls' falls the clearance laws' (P1); the planner's part is measured with P5 in roadmap item 18 stage 0. Decided 2026-09-25: the investigation first; repaired in part on 2026-09-26. | 32 to 52 flips per camera flight, 25 to 34 per lidar flight; the return repaired, the falls remain | (b) | investigations goal 2026-09-25; S4 goal 2026-09-26 |

## Camera Perception

| Debt | Measured | Class | Found by |
|---|---|---|---|
| A faced motion whose elevation lies between the pair's vertical field and a time-of-flight cone is seen by no sensor and is flown at the vertical contract's speed. Measured on the camera series r607 to r611 the band holds 1.7 percent of the moving time and 0.9 percent of the path, 28 s over five flights at 1.0 m/s, every episode 0.2 to 0.8 s long and inside a shaft, so its own price is about 2 percent of the mean speed. What the band hides is larger and sits in the shafts: climbing one, the pair faced one or two of eight heading sectors per half metre, the walls it did not see stayed unknown, and the planner went on hoping for exits in them; the shaft at (53, -7) m, 30 m from the goal, was tried on five of the eight camera flights r599 to r611 and cost 27 to 50 s each (two climbs through 34 to 39 routes, three abandonments with an 80 m detour), about 34 s a flight. 3df703ce turns the gaze at 1.5 rad/s while a climb or a descent has no heading to face (`gaze_survey_yaw_rate_radps`), which gives the pair 99 to 100 percent of a shaft's walls per half metre against 60 to 78 percent before (r617 to r622 against r608 to r611 in the shaft at (54, -25)); the shaft at (53, -7) was then chosen on r628 to r630 and r635 to r637 with the survey on, and its price did not move (r628: 46 s and 23 routes with 95 percent of the walls seen; the exit at z 15 was seen and reached at t = 225 s and the route left it for the unknown above, z 18, a ceiling; r630 27 s; r629, r636, r637 abandoned after 21 to 40 s). What the survey cannot close is the planner's hope for the unknown above the shaft, which the invariant keeps free; seeing the ceiling from below is a sensor on the vehicle (the time-of-flight unit sees 2.8 m) or a wider field. Decided 2026-09-25: the survey stays for what it observes, route shaping is not pursued (the band lives in shafts, where a diagonal has no room). | band 52.4 to 67.5 degrees, 1.74 percent of the moving time at 1.0 m/s; the shaft 27 to 50 s on five of eight flights before the survey | (c) | item 14, C1 follow-up 2026-09-25 |
| Lateral deviations of the horizon from the route enter unknown space at the validators' speed; under the lidar that space had already been seen. Measured in roadmap item 17 stage 4: the time-of-flight ring gives the contract a lateral range of 4 m, so a deviation beside the vehicle enters space the ring has looked at; what the ring leaves unmeasured is decided on that number. | not measured as a separate figure | (b) | item 14 |
...
| The planning loop runs near 43 Hz with most ticks over the 20 ms deadline and no single bottleneck left; the planner spends its whole 150 ms budget, so its p95 measures the configuration. Re-measured in roadmap item 20 stage 0, which moves the tick to the node clock and reports the tick and planner percentiles beside these; the deadline question is decided on those numbers, and the rework stays its own. | tick 22 to 23 ms at p50, 30 to 37 ms at p95 | (b) | item 10 |
| The persistent planner's update has no bound on its own stages but the deadline the departure and goal probing now respect. On r596 (2026-09-25, camera profile, f4c45394) the vehicle stood 106 s at (5.5, 21, 17) m under the blocked-route law of a stale route: every update spent 4.4 s (search_ms p50 4382, raw sweeps 4276 ms, repair and feasibility 0) in the departure refinement, 512 probes at 8 ms of raw sweep each, and the queued continuation was retired every tick, so the route was never replaced while the raw world advanced a revision on nearly every tick; the flight timed out 45 m from the goal after an 816 m excursion. f0d64d86 stops both probings at the update's deadline (telemetry `departure_deadline_hit`, never hit on the twenty acceptance flights r597 to r616, planner p95 152 to 163 ms). What remains is that the planner's budget between repair, feasibility search and world changes is not scheduled: a stage that finds no stable world can still spend the whole update, and route stall recovery (`route_stall_recovery_enabled`, off) is the existing lever to measure against it. Lands in roadmap item 18 stage 0 as its first step: transient occupancy and decay multiply the occupancy churn that triggered r596, so the budget between repair, feasibility search and world changes is scheduled before the memory starts to forget; the falls of S4 (a route every 0.4 to 0.5 s, the no-route gaps) are measured there with it. | one flight in eleven before the repair; none in twenty after | (b) | speed goal 2026-09-24 |
| The onboard loop runs on the wall clock while the world runs on the simulation clock. The planning tick is `create_wall_timer(1 / tick_rate_hz)` (`production_mppi_node_runtime.cpp`) and the offboard tick `create_wall_timer(20 ms)` (`mppi_offboard_node.cpp`); twelve more wall timers sit in the mission monitor, the cooperative agent and referee, the spectator, the diagnostics mux, the truth adapter, the lidar debug node and the node interfaces, and 192 uses of `steady_clock` in 66 files serve budgets and latency measurements, with a few freshness watchdogs among them (`planner_health_received_at_`). Every node declares `use_sim_time` for its stamps and none for its timers. So a simulation slower than real time flies the vehicle with a faster computer than it has (item 15: about 72 planning ticks per simulated second at a real-time factor of 0.6 against 43), and slowing the world on purpose lightens the host only by what runs on the simulation clock: four vehicles' planning nodes alone are 7.4 of the 8 physical cores at any factor. Planned in roadmap item 20, stage 0: the two ticks move to the node clock, the other timers and the watchdogs are sorted into what keeps simulation time and what stays wall-clock, the flight window of the scripts (`SMOKE_DURATION_S`, wall-clock) follows the factor, and both acceptance series are flown at a factor of 1.0 with the tick and planner percentiles reported beside these. Estimated M: the code is S, the audit and the acceptance are the rest. | 72 against 43 ticks per simulated second at a factor of 0.6; 14 wall timers; 4 x 1.86 cores of planning at any factor | (b) | item 15, item 20 |
...
been flown from one start to one goal. Where each lands: the cooperative flights are item 15's, on the lidar profile by its decision, so the camera defaults for several vehicles stay unflown; the cave and the finals locations are flown by item 11's survey acquisition; item 19 flies the mission back and to a goal outside the map, the first flights with another goal and another direction.
