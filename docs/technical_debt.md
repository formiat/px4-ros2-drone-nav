...
| The hover's rest clearance | The clearance goes from 0.27 to 0.35 m, the autopilot's rest error at p95 plus the estimate's slide at p95 (0.15 + 0.14 on the camera profile), the only direction the invariant allows; its price in speed beside walls is measured with both series. | a parameter change with both series |
...
| The braking laws' flicker | An investigation on the records first, flip by flip: whether the fall carries a measurement (a wall or the range shortened) or the sweep of the pair's field and the route's replacement; a repair, if any, follows the finding, and a hysteresis on the return is not applied blind, since it can only take speed. | the investigation, then a decision |
...
| S4 the braking laws' flicker | no | medium: 5 percent of a camera flight, 14 percent of a lidar flight | high: a law's return and the swing after it touch the first requirement | low: slow but safe | decided, see above: the investigation on the records first | M plus both series | the speed laws; the mean speed |
