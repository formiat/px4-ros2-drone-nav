...
| S4 the braking laws' flicker | no | medium: 5 percent of a camera flight, 14 percent of a lidar flight | high: a law's return and the swing after it touch the first requirement | low: slow but safe | not known: the return of a law and the jerk swing after it; the falls are measurements, item 17 reads their inputs | M plus both series | the speed laws; the mean speed |
...
| P3 the hover drifts past the rest clearance | no | medium | medium: one number, the price is speed beside walls | low: 0.27 m covers the autopilot's rest error at p95 (0.15 cameras, 0.20 lidar), not its maximum (0.32) nor the sum with the estimate's slide (0.29 at p95); no contact in thirty flights | measured 2026-09-25: the clearance raised to about 0.35 m with both series, or the p95 coverage accepted; the owner's | S plus both series | the rest pose beside walls; a little speed |
...
| P6 the recovery at 2 m/s^2 against the 4 admitted | no | low: 0.9 to 1.0 s per flight, under 1 percent | low: sixteen lines flown once (3326013f) | none | measured 2026-09-25: the cubic connector's law on straight intervals; the guide seed for them with both series, or left as the connector's price (recommended); the owner's | S plus both series | the route-directed seed; a second a flight |
...
| The braking laws flicker: the reference falls by more than 1 m/s and returns within 2 s 32 to 52 times per camera flight (p50 0.35 s long; the sensor-braking law at the bottom of 60 to 75 percent of them) and 25 to 34 times per lidar flight (p50 0.78 s; the clearance and route-clearance laws), and after each the acceleration swings back at the control jerk, 0.67 s from -4 to +4 m/s^2, while the reference is already back. Their price, the speed deficit over the flip and its recovery (reference_flips.py, r632 to r642): 7 to 18 s per camera flight (mean 13.4 s, 5 percent of the flight) and 14 to 29 s per lidar flight (mean 20.6 s, 14 percent). Each fall is a law reading a measurement, and the safety direction forbids smoothing the fall; what may be looked at is the return, the swing after it, and whether a fall of a few ticks carries a measurement or the sweep of the pair's field (item 17 reads the contract's inputs). | 32 to 52 flips per camera flight, 13 s; 25 to 34 per lidar flight, 21 s | (c) | investigations goal 2026-09-25 |
...
| A holding vehicle's rest clearance, 0.27 m beyond the body, covers the autopilot's error at rest at p95 but not at its maximum nor together with the estimate's slide. Measured on the twenty acceptance flights of 2026-09-25 (investigations goal; hold_rest_error.py, rest_tracking_error.py): the autopilot's instantaneous error against the rest setpoint it is given, in its own frame, is p50 0.03 / p95 0.15 / max 0.32 m on the camera profile (3815 samples; the maximum r611's settling at the goal) and 0.06 / 0.20 / 0.27 m on the lidar profile; within a rest episode the estimate slides against the truth by p95 0.14 m (cameras) and 0.17 m (lidar); hold poses do not walk (one re-anchoring step of 0.2 m in twenty flights). The register's former 0.37 m at p95 was the takeoff spool-up (t 10 to 12 s, 0.31 to 0.38 m on every flight, the vehicle rising below 0.25 m/s) counted as a hover by the episode definition of hold_drift_truth.py; the true hovers of r303 to r334 sit at p50 0.10 m. Three laws that released the margin were flown and reverted (2026-09-16) and the margin is not released here either. The owner's decision: raise the rest clearance to about 0.35 m (the autopilot's p95 and the slide's p95 at once, 0.29 m, or the 0.32 m maximum) at a price in speed to measure with both series, or accept the p95 coverage with these numbers (no contact in thirty flights). | autopilot rest error p95 0.15 / max 0.32 m (cameras), 0.20 / 0.27 (lidar); estimate slide p95 0.14 / 0.17 m; clearance 0.27 m | (c) | item 10; investigations goal 2026-09-25 |
...
| The recovery after a stop runs at about 2 m/s^2 on half of the straight recovery ticks against the 4 m/s^2 the profile admits. Cause (investigations goal 2026-09-25, on the executed first control of the full tick record over r632 to r642): on a straight route interval the route-directed seed is the rest-to-rest cubic connector to a terminal half a horizon ahead, whose duration the speed bound pins at 0.75 of the horizon (3.0 s), so its first control is (4 / 3)(reference - speed), a first-order rise with a 0.75 s time constant: 3.2 m/s^2 from rest at 2.4 m/s, 1.4 m/s^2 at the median of the ticks it governs (the executed control over the law p50 1.07, p10 0.95, p90 1.33, n = 255). It governs 52 percent of the straight recovery ticks; the other 48 percent (the guide seed on curved intervals, the connector from rest) run at 3.9. The repair is known and local, the guide seed for straight non-terminal intervals (3326013f, flown on r594, where the law's ticks fell from 15 to 1, and reverted for a mean speed that did not move), and it did not move because the law costs 0.4 to 1.5 s per camera flight (mean 0.9 s, 0.4 percent) and 0.4 to 2.5 s per lidar flight (mean 1.0 s, 0.7 percent). The budget's 0.96 m/s^2 over 173 brakings was not an acceleration figure: it mixed the wait for the heading (0.45 m/s^2 with the heading 90 degrees off, speed goal), the reversals after route replacements (37 to 51 percent of the ticks that brake against a high reference) and the jerk-bound swing of the acceleration after every flip of a braking law (12 m/s^3: 0.67 s from -4 to +4 m/s^2; the flips are the entry above in Speed). Excluded on the records: the reference slew and profile, the autopilot (speed error p50 0.04 m/s, lag 0.04 s), the MPPI cost (the candidate executed unrepaired on 96 percent of the ticks), the vector clamp (peak 3.9 to 4.2 when the seed asks). The track's `first_control` field is the cleared diagnostic copy and always 0; the full record's `first_control_*_mps2` is the executed control. The owner's decision: a series pair for the guide seed at a second per flight (not recommended), or the entry left as the connector's price. | (4 / 3)(reference - speed) on 52 percent of the straight recovery ticks, 1.07 x the law; 0.9 to 1.0 s per flight | (c) | speed goal 2026-09-24; investigations goal 2026-09-25 |
