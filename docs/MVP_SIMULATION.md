...
`/drone_city_nav/path`, and `/drone_city_nav/lidar_debug_points`. The point
cloud is generated by `lidar_debug_node` in the `map` frame, so it does not
depend on a Gazebo lidar TF tree.
