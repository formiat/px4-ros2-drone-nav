...
The received executable trajectory samples include `z_m`, and offboard keeps
that altitude in accepted samples, final trajectory RViz publication, dumps,
and blackbox projection diagnostics. Runtime vertical control follows the
trajectory vertical profile when one is present. During terminal position
