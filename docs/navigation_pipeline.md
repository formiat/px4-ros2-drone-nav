...
physical evidence ends its path's claim has no stop to fly: the rest position
is held explicitly (`EXECUTION_HOLD rest=true`), a stationary hold that
supersedes the stale horizon on the wire, and the next certified route takes
the vehicle back from it. A vehicle still moving whose stop cannot be certified
— the braking path itself sweeps into the evidence — is revoked: the resident
route's finite execution is suspended, the offboard is told, and it holds the
position it is at. Both used to leave the offboard flying the last horizon it
had accepted, certified before the evidence that had just invalidated it, to
that horizon's rest point — a revocation could not be committed while a route
was resident, because the plan a suspension leaves has no horizon to publish —
and two recorded flights ended against walls that way. A detached revocation
leases no horizon, so it commits a plan without one. A route may also take the vehicle back
